...
export function AppShell({ email, children }: { email: string; children: React.ReactNode }) {
  ...
  return <div className="min-h-dvh">
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-[248px] border-r bg-[var(--card)]/80 p-4 backdrop-blur-xl lg:flex lg:flex-col">
      ...
      <div className="mt-auto rounded-3xl bg-[var(--sage-soft)] p-4"><div className="mb-3 grid size-9 place-items-center rounded-xl bg-white/60 text-[var(--sage)] dark:bg-white/10"><HeartPulse className="size-5" /></div><p className="text-sm font-semibold">小小提醒</p><p className="mt-1 text-xs leading-relaxed text-[var(--muted)]">记录不必完美，留下当时的心情就很好。</p></div>
    </aside>
