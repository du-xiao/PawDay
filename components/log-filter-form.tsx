...
export function LogFilterForm({ q, type, pet, pets, types }: { q: string; type: string; pet: string; pets: PetOption[]; types: string[] }) {
  ...
  return (
    <form ref={formRef} className="soft-card mb-6 rounded-3xl p-3 sm:p-4">
      <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_11rem_12rem]">
        ...
        <div className="relative rounded-2xl bg-black/[.025] dark:bg-white/[.04]">
          <PawPrint className="pointer-events-none absolute left-4 top-1/2 z-10 size-4 -translate-y-1/2 text-[var(--muted)]" />
          <select
            name="pet"
            defaultValue={pet}
            aria-label="宠物"
          ...
          >
            ...
            {pets.map((item) => <option key={item.id} value={item.id}>{petOptionLabel(item)}</option>)}
          </select>
        </div>
      </div>
