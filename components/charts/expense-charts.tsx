...
function ResponsiveContainer(props: ComponentProps<typeof RechartsResponsiveContainer>) {
  return <RechartsResponsiveContainer initialDimension={{ width: 320, height: 256 }} {...props} />;
}
...
export function ExpenseCharts({categories,months,scope,selectedYear,selectedMonth,years}:ExpenseChartsProps){
  ...
  const periodKey=`${scope}-${selectedYear}-${selectedMonth}-${categories.map((item)=>`${item.name}:${item.value}`).join("|")}`;
  ...
  return <div className="grid min-w-0 gap-4 sm:gap-5 xl:grid-cols-2">
    <div className="soft-card min-w-0 overflow-hidden rounded-2xl p-4 sm:rounded-3xl sm:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4"><div><h3 className="font-semibold">分类占比</h3><p className="mt-1 text-xs text-[var(--muted)]">{periodLabel} · 合计 {money(categoryTotal)}</p></div><div className="flex rounded-xl bg-black/[.035] p-1 dark:bg-white/[.05]"><button type="button" onClick={()=>update({scope:"month",year:String(selectedYear),month:String(selectedMonth)})} className={`rounded-lg px-3 py-1.5 text-xs font-medium transition ${scope==="month"?"bg-[var(--background)] text-[var(--orange)] shadow-sm":"text-[var(--muted)]"}`}>按月</button><button type="button" onClick={()=>update({scope:"year",year:String(selectedYear)})} className={`rounded-lg px-3 py-1.5 text-xs font-medium transition ${scope==="year"?"bg-[var(--background)] text-[var(--orange)] shadow-sm":"text-[var(--muted)]"}`}>按年</button></div></div>
      <div className="mt-4 flex gap-2"><select aria-label="选择年份" value={selectedYear} onChange={event=>update({year:event.target.value})} className="h-9 rounded-xl border bg-white/60 px-3 text-xs font-medium outline-none focus:border-orange-300 dark:bg-white/[.05]">{years.map(year=><option key={year} value={year}>{year} 年</option>)}</select>{scope==="month"&&<select aria-label="选择月份" value={selectedMonth} onChange={event=>update({scope:"month",month:event.target.value})} className="h-9 rounded-xl border bg-white/60 px-3 text-xs font-medium outline-none focus:border-orange-300 dark:bg-white/[.05]">{Array.from({length:12},(_,index)=>index+1).map(month=><option key={month} value={month}>{month} 月</option>)}</select>}</div>
      {categories.length?<div key={periodKey} className="mt-2 grid min-w-0 items-center sm:grid-cols-[1fr_150px]"><div className="h-48 min-w-0 sm:h-56"><ResponsiveContainer key={periodKey} minWidth={0}><PieChart><Pie isAnimationActive={false} data={categories} dataKey="value" nameKey="name" innerRadius={42} outerRadius={70} paddingAngle={3}>{categories.map((_,i)=><Cell key={i} fill={colors[i%colors.length]}/>)}</Pie><Tooltip formatter={(value)=>money(Number(value))} contentStyle={{borderRadius:16,border:"1px solid var(--line)",background:"var(--background)",fontSize:12}}/></PieChart></ResponsiveContainer></div><div className="space-y-2">{categories.slice(0,6).map((item,i)=><div key={item.name} className="flex items-center justify-between gap-3 text-xs"><span className="flex items-center gap-2"><i className="size-2 rounded-full" style={{background:colors[i%colors.length]}}/>{item.name}</span><span className="text-[var(--muted)]">{money(item.value)}</span></div>)}</div></div>:<div key={periodKey} className="grid h-48 place-items-center text-center text-sm text-[var(--muted)] sm:h-56">{periodLabel} 暂无开销数据</div>}
    </div>
    <div className="soft-card min-w-0 overflow-hidden rounded-2xl p-4 sm:rounded-3xl sm:p-6">
      <h3 className="font-semibold">月度趋势</h3><p className="mt-1 text-xs text-[var(--muted)]">过去 12 个月</p>
      <div className="mt-4 h-56 min-w-0 sm:mt-6 sm:h-64"><ResponsiveContainer minWidth={0}><BarChart data={months} margin={{left:-8,right:4}}><CartesianGrid vertical={false} stroke="var(--line)"/><XAxis dataKey="month" axisLine={false} tickLine={false} tick={{fontSize:10,fill:"var(--muted)"}}/><YAxis width={46} axisLine={false} tickLine={false} tick={{fontSize:10,fill:"var(--muted)"}} tickFormatter={(value)=>`¥${Math.round(Number(value)/100)}`}/><Tooltip formatter={(value)=>[money(Number(value)),"费用"]} contentStyle={{borderRadius:16,border:"1px solid var(--line)",background:"var(--background)",fontSize:12}}/><Bar dataKey="value" name="费用" fill="#e88758" radius={[8,8,3,3]} maxBarSize={34}/></BarChart></ResponsiveContainer></div>
    </div>
  </div>;
}
