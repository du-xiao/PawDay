...
export function FormActions({ pending, submitLabel = "保存记录", onCancel }: { pending: boolean; submitLabel?: string; onCancel?: () => void }) {
  return <div className="mt-7 flex justify-end gap-3">{onCancel && <Button type="button" variant="ghost" onClick={onCancel}>取消</Button>}<Button type="submit" variant="warm" disabled={pending}>{pending && <LoaderCircle className="size-4 animate-spin" />}{submitLabel}</Button></div>;
}
