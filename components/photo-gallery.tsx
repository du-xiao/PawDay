...
import { CalendarDays, ChevronLeft, ChevronRight, Link2, X } from "lucide-react";
...
export function PhotoGallery({ photos, canWrite = true }: { photos: Photo[]; canWrite?: boolean }) {
  ...
  const nextPhoto = activeIndex >= 0 && activeIndex < photos.length - 1 ? photos[activeIndex + 1] : null;
  ...
  return <>
      ...
      {groups.map((group) => <section key={group.key}>
        ...
        <div className="columns-2 gap-2 sm:columns-3 sm:gap-3 lg:columns-4 xl:columns-5">
          {group.photos.map((photo, index) => <button key={photo.id} onClick={() => setActive(photo)} className="group relative mb-2 block w-full overflow-hidden rounded-2xl bg-black/[.05] text-left shadow-sm transition hover:-translate-y-0.5 hover:shadow-xl sm:mb-3 sm:rounded-3xl dark:bg-white/[.04]">
            ...
            <span className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/75 via-black/25 to-transparent p-3 pt-12 text-white transition sm:p-4 sm:pt-16 sm:opacity-0 sm:group-hover:opacity-100">
              <span className="block line-clamp-1 text-sm font-medium">{photo.title || formatDate(photo.date, "M月d日")}</span>
              <span className="mt-0.5 block text-[11px] text-white/75">{formatDate(photo.date)}</span>
            </span>
          </button>)}
        </div>
      </section>)}
    ...
    {active && typeof document !== "undefined" && createPortal(<div className="fixed inset-0 z-50 grid place-items-center bg-black/75 p-2 backdrop-blur-md sm:p-3" onClick={() => setActive(null)}>
      ...
      <div className="grid max-h-[94vh] w-full max-w-5xl overflow-hidden rounded-2xl bg-[var(--background)] shadow-2xl sm:rounded-3xl lg:grid-cols-[minmax(0,1fr)_320px]" onClick={(event) => event.stopPropagation()}>
        <div className="relative min-h-[46vh] bg-black lg:min-h-[70vh]">
          <Image src={imageVariantUrl(active.url, "medium")} alt={active.title || "小狗照片"} fill unoptimized className="object-contain" sizes="80vw" />
          {previousPhoto && <button type="button" onClick={() => setActive(previousPhoto)} className="absolute left-3 top-1/2 grid size-10 -translate-y-1/2 place-items-center rounded-full bg-black/35 text-white backdrop-blur transition hover:bg-black/50"><ChevronLeft className="size-5" /><span className="sr-only">上一张</span></button>}
          {nextPhoto && <button type="button" onClick={() => setActive(nextPhoto)} className="absolute right-3 top-1/2 grid size-10 -translate-y-1/2 place-items-center rounded-full bg-black/35 text-white backdrop-blur transition hover:bg-black/50"><ChevronRight className="size-5" /><span className="sr-only">下一张</span></button>}
        </div>
