...
PawDay 是一个为家庭 NAS 设计的私人宠物生活记录应用。它支持多宠物家庭为猫咪和狗狗分别建立档案，把日常、健康、开销和照片放在一个温暖、移动端友好的空间里，全部数据保存在自己的设备上。

## 功能

- 主人邮箱密码登录，支持访客只读账号，Auth.js 会话有效期 30 天
- 首次启动自动创建管理员账号与 SQLite 表结构，旧库升级会自动补齐新增字段
- 多宠物档案，支持猫咪 / 狗狗、年龄、陪伴天数、头像和证件
- 日常记录的新增、编辑、删除、搜索、类型筛选、宠物筛选和图片
- 养宠开销统计、分类占比、月度趋势和宠物筛选
- 健康时间线、单宠体重趋势、宠物筛选和下次提醒
- JPG / PNG / WebP 照片上传、灯箱预览、宠物归属和日常记录关联
- 深色模式、桌面侧栏、移动端底部导航
...
应用第一次收到请求时会自动建表，并且只在数据库没有用户时创建一个管理员。不会自动创建宠物、日常、开销、健康、提醒或照片数据。`pnpm db:seed` 也只执行同一套管理员初始化逻辑。

`ADMIN_EMAIL` 和 `ADMIN_PASSWORD` 只用于空数据库的首次初始化。数据库已有管理员后，修改环境变量不会覆盖现有账号或密码，密码请在应用的“设置”页面修改。

访客账号是可选的只读账号。管理员可以在“设置”页面创建、停用或修改访客账号；也可以通过 `GUEST_ENABLED`、`GUEST_EMAIL`、`GUEST_PASSWORD` 在环境变量中预置。访客只能查看信息，不能新增、修改、删除或修改密码。
...
- `GUEST_ENABLED`：是否通过环境变量同步访客账号，留空表示交给设置页管理，`true` 启用，`false` 停用
- `GUEST_EMAIL`：访客邮箱，可留空后由管理员在设置页创建
- `GUEST_PASSWORD`：访客密码，首次通过环境变量创建时至少 8 位
...
访客环境变量在每次容器启动时都会同步：`GUEST_ENABLED=true` 会创建或启用访客账号，`GUEST_ENABLED=false` 会停用访客账号。`GUEST_ENABLED` 留空且不填写访客邮箱/密码时，应用不会自动改动访客账号，可以直接在“设置”页面由管理员管理。访客账号只读，所有写操作都会被服务端拒绝。
