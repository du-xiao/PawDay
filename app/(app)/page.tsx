...
const logColors:Record<string,string>={"喂食":"bg-amber-500/10 text-amber-700 dark:text-amber-300","遛狗":"bg-emerald-500/10 text-emerald-700 dark:text-emerald-300","洗澡":"bg-sky-500/10 text-sky-700 dark:text-sky-300","睡眠":"bg-violet-500/10 text-violet-700 dark:text-violet-300","训练":"bg-orange-500/10 text-orange-700 dark:text-orange-300"};
...
export default async function DashboardPage(){
  ...
  return <div className="page-enter space-y-6 sm:space-y-8">
    <section className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-medium text-[var(--orange)]">{new Intl.DateTimeFormat("zh-CN",{month:"long",day:"numeric",weekday:"long"}).format(now)}</p><h1 className="mt-2 text-3xl font-semibold tracking-[-.05em] sm:text-5xl">今天也要好好生活。</h1><p className="mt-3 text-[var(--muted)]">陪 {dog.name} 认真度过普通的一天。</p></div><div className="w-full rounded-2xl border border-orange-200/35 bg-[var(--orange-soft)]/55 p-1.5 shadow-sm shadow-orange-200/20 sm:w-auto sm:border-0 sm:bg-transparent sm:p-0 sm:shadow-none [&>button]:w-full sm:[&>button]:w-auto"><LogForm/></div></section>
    <section className="relative overflow-hidden rounded-[2rem] bg-gradient-to-br from-[#e9a476] via-[#e8895f] to-[#cc705b] p-6 text-white shadow-xl shadow-orange-300/20 sm:p-8"><div className="relative z-10 flex flex-col gap-7 sm:flex-row sm:items-center"><div className="relative size-24 shrink-0 overflow-hidden rounded-[1.75rem] border-4 border-white/30 bg-white/20 sm:size-28">{dog.avatarUrl?<Image src={dog.avatarUrl} alt={dog.name} fill priority unoptimized className="object-cover" sizes="112px"/>:<div className="grid size-full place-items-center"><PawPrint className="size-10"/></div>}</div><div><p className="text-sm text-white/75">MY BEST FRIEND</p><h2 className="mt-1 text-3xl font-semibold tracking-[-.04em]">{dog.name}</h2><div className="mt-3 flex flex-wrap gap-2"><Badge className="bg-white/20 text-white">{dog.breed||"品种未记录"}</Badge><Badge className="bg-white/20 text-white">{dogAge(dog.birthDate)}</Badge>{together&&<Badge className="bg-white/20 text-white">陪伴第 {together} 天</Badge>}</div></div><Button asChild variant="outline" className="border-white/25 bg-white/15 text-white hover:bg-white/20 sm:ml-auto"><Link href="/dog">查看档案<ArrowUpRight className="size-4"/></Link></Button></div><PawPrint className="absolute -bottom-14 -right-6 size-52 rotate-[-18deg] text-white/[.07]"/></section>
