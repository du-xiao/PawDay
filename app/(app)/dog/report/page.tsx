...
export default async function DogReportPage() {
  ...
  return (
    <div className="page-enter">
      ...
      <article className="print-report mx-auto max-w-5xl rounded-[1.5rem] border bg-white p-6 text-[#24211d] shadow-xl shadow-stone-900/[.06] sm:p-8 print:shadow-none">
        <header className="flex flex-col gap-5 border-b border-stone-200 pb-6 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            <div className="relative size-24 shrink-0 overflow-hidden rounded-2xl border border-stone-200 bg-stone-100">
              {dog.avatarUrl ? <Image src={imageVariantUrl(dog.avatarUrl, "thumb")} alt={dog.name} fill unoptimized className="object-cover" sizes="96px" /> : <div className="grid size-full place-items-center text-stone-400"><HeartPulse className="size-9" /></div>}
            </div>
            <div>
              <p className="text-xs font-semibold uppercase text-[#8d5a3c]">PawDay Health Report</p>
              <h1 className="mt-1 text-3xl font-semibold">{dog.name}</h1>
...
function Fact({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl bg-stone-100 px-3 py-2"><p className="text-[11px] text-stone-500">{label}</p><p className="mt-0.5 truncate font-semibold">{value}</p></div>;
}

function ReportBlock({ icon: Icon, title, children }: { icon: typeof Scale; title: string; children: React.ReactNode }) {
  return <section className="rounded-2xl border border-stone-200 p-4"><ReportTitle icon={Icon} title={title} />{children}</section>;
}

function ReportTitle({ icon: Icon, title }: { icon: typeof Scale; title: string }) {
  return <div className="mb-3 flex items-center gap-2"><span className="grid size-8 place-items-center rounded-xl bg-[#f6ddcc] text-[#9a5838]"><Icon className="size-4" /></span><h2 className="font-semibold">{title}</h2></div>;
}
  ...
  return (
    <section className="rounded-2xl border border-stone-200 p-4">
      ...
      {rows.length ? <div className="overflow-hidden rounded-xl border border-stone-200">
        {rows.map((row, index) => <div key={`${row[0]}-${index}`} className="grid grid-cols-[7.5rem_1fr] gap-3 border-b border-stone-200 px-3 py-2 text-sm last:border-b-0 sm:grid-cols-[8rem_1fr_1.2fr]"><span className="text-stone-500">{row[0]}</span><span className="font-medium">{row[1]}</span><span className="col-span-2 text-stone-500 sm:col-span-1">{row[2] || "-"}</span></div>)}
