...
mkdir -p /data /uploads /app/.next/cache "$TMPDIR"
...
if ! su-exec "${PUID}:${PGID}" sh -c '
  ...
  temp_probe="${TMPDIR:-/tmp}/.pawday-write-test-$$"
...
'; then
  echo "运行目录不可写，请检查 /data、/uploads、/app/.next/cache、${TMPDIR} 以及 PUID/PGID 权限" >&2
  exit 1
fi
